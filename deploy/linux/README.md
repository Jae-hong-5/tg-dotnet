...
아이콘(`AppIcon-256.png`), 데스크톱 엔트리(`TimeGrapher.App.desktop`)가 함께 들어 있다.
...
cd ~/timegrapher
chmod +x ./TimeGrapher.App   # tar가 +x를 보존하지만 umask 대비 한 번 더
./TimeGrapher.App
```

self-contained 빌드라 **.NET 런타임은 설치할 필요 없다.** 다만 GUI·폰트용 *시스템*
라이브러리는 신선한 Pi OS에 없을 수 있다(아래 2번).

## 2. 런타임 의존성 (신선한 Pi OS에서 한 번만)
